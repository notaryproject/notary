...
func TestCDBTUFFileJSONUnmarshalling(t *testing.T) {
	...
	unmarshalledAnon, err := TUFFilesCouchTable.JSONUnmarshaller(jsonBytes)
	require.NoError(t, err)
	unmarshalled, ok := unmarshalledAnon.(CDBTUFFile)
	require.True(t, ok)
...
func TestCDBTUFFileJSONUnmarshallingFailure(t *testing.T) {
	...
	for _, invalid := range invalids {
		_, err := TUFFilesCouchTable.JSONUnmarshaller([]byte(invalid))
		require.Error(t, err)
	}
}
