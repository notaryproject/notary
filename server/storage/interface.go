...
type MetaStore interface {
	...
	UpdateCurrent(gun data.GUN, namespace Namespace, update MetaUpdate) error
	...
	UpdateMany(gun data.GUN, namespace Namespace, updates []MetaUpdate) error
	...
	GetCurrent(gun data.GUN, namespace Namespace, tufRole data.RoleName) (created *time.Time, data []byte, err error)
	...
	GetChecksum(gun data.GUN, namespace Namespace, tufRole data.RoleName, checksum string) (created *time.Time, data []byte, err error)
	...
	// not found, it returns storage.ErrNotFound
	GetVersion(gun data.GUN, namespace Namespace, tufRole data.RoleName, version int) (created *time.Time, data []byte, err error)

	// Delete removes all metadata for a given GUN.  It does not return an
	// error if no metadata exists for the given GUN.
	Delete(gun data.GUN, namespace Namespace) error
