...
func metaFromRepo(t *testing.T, gun data.GUN, version int) map[string]StoredTUFMeta {
	...
	for role, tufdata := range metaBytes {
		tufMeta[role.String()] = SampleCustomTUFObj(gun, role, version, DefaultNamespace, tufdata)
	}
...
func testTUFMetaStoreGetCurrent(t *testing.T, s MetaStore) {
	...
	initialRootTUF := SampleCustomTUFObj(gun, data.CanonicalRootRole, 1, DefaultNamespace, nil)
	...
	require.NoError(t, s.UpdateCurrent(gun, DefaultNamespace, MakeUpdate(initialRootTUF)), "Adding root to empty store failed.")
	...
	_, _, err := tufDBStore.GetChecksum(gun, DefaultNamespace, data.CanonicalRootRole, initialRootTUF.SHA256)
	require.NoError(t, err)
	...
	require.NoError(t, s.Delete(gun, DefaultNamespace), "unable to delete metadata")
	...
	require.NoError(t, s.UpdateMany(gun, DefaultNamespace, updates))
	_, _, err = s.GetCurrent(gun, DefaultNamespace, data.CanonicalSnapshotRole)
	...
	orphanedRootTUF := SampleCustomTUFObj(gun, data.CanonicalRootRole, 3, DefaultNamespace, []byte("orphanedRoot"))
	require.NoError(t, s.UpdateCurrent(gun, DefaultNamespace, MakeUpdate(orphanedRootTUF)), "unable to create orphaned root in store")
...
func ConsistentGetCurrentFoundTest(t *testing.T, s *TUFMetaStorage, rec StoredTUFMeta) {
	_, byt, err := s.GetCurrent(rec.Gun, DefaultNamespace, rec.Role)
	require.NoError(t, err)
...
func ConsistentEmptyGetCurrentTest(t *testing.T, s *TUFMetaStorage, rec StoredTUFMeta) {
	...
	_, byt, err = s.MetaStore.GetCurrent(rec.Gun, DefaultNamespace, rec.Role)
	require.Nil(t, byt)
	require.Error(t, err, "There should be an error getting an empty table")
	require.IsType(t, ErrNotFound{}, err, "Should get a not found error")
}
