...
func GetOrCreateTimestamp(gun data.GUN, store storage.MetaStore, cryptoService signed.CryptoService) (
	*time.Time, []byte, error) {
	...
	lastModified, timestampJSON, err := store.GetCurrent(gun, storage.DefaultNamespace, data.CanonicalTimestampRole)
	if err != nil {
	...
	if err = store.UpdateMany(gun, storage.DefaultNamespace, updates); err != nil {
		return nil, nil, err
	}
...
func createTimestamp(gun data.GUN, prev *data.SignedTimestamp, snapshot []byte, store storage.MetaStore,
	...
	_, root, err := store.GetCurrent(gun, storage.DefaultNamespace, data.CanonicalRootRole)
	if err != nil {
