...
func TestCDBPrivateKeyJSONUnmarshalling(t *testing.T) {
	...
	unmarshalledAnon, err := PrivateKeysCouchTable.JSONUnmarshaller(jsonBytes)
	require.NoError(t, err)
	unmarshalled, ok := unmarshalledAnon.(CDBPrivateKey)
	require.True(t, ok)
	...
	// the timing struct to compare the rest of the CDBTUFFile struct
...
func TestCDBPrivateKeyJSONUnmarshallingFailure(t *testing.T) {
	...
	_, err = PrivateKeysCouchTable.JSONUnmarshaller([]byte(invalid))
	require.Error(t, err)
}
