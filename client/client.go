...
func NewNotaryRepository(baseDir, gun, baseURL string, remoteStore store.RemoteStore, cache store.MetadataStore,
...
func (r *NotaryRepository) initializeRoles(rootKeys []data.PublicKey, localRoles, remoteRoles []string) (
	...
	for _, role := range remoteRoles {
		...
		var key data.PublicKey
		key, err = getRemoteKey(role, r.remoteStore)
		if err != nil {
			return
		}
...
func addChange(cl changelist.Changelist, c changelist.Change, roles ...string) error {
...
func (r *NotaryRepository) pubKeyListForRotation(role string, serverManaged bool, newKeys []string) (pubKeyList data.KeyList, err error) {
	var pubKey data.PublicKey

	// If server manages the key being rotated, request a rotation and return the new key
	if serverManaged {
		pubKey, err = rotateRemoteKey(role, r.remoteStore)
...
func (r *NotaryRepository) pubKeysToCerts(role string, pubKeyList data.KeyList) (data.KeyList, error) {
...
func DeleteTrustData(baseDir, gun, URL string, rt http.RoundTripper, deleteRemote bool) error {
	localRepo := filepath.Join(baseDir, tufDir, filepath.FromSlash(gun))
