...
func TestSetupGRPCServer(t *testing.T) {
	v := viper.New()
	v.SetDefault("backend", notary.MemoryBackend)
	s, err := setupGRPCServer(v)
	require.NoError(t, err)
	require.IsType(t, grpc.NewServer(), s)

	v = viper.New()
	v.SetDefault("backend", "not recognized")
...
func TestSetupStorage(t *testing.T) {
	...
	v = viper.New()
	v.SetDefault("backend", notary.FileBackend)
	s, err = setupStorage(v)
	require.NoError(t, err)
...
func TestSetupNetListener(t *testing.T) {
	v := viper.New()
	v.SetDefault("addr", "invalidaddr")
	_, err := setupNetListener(v)
	require.Error(t, err)

	v = viper.New()
	v.SetDefault("addr", "127.0.0.1:9999")
