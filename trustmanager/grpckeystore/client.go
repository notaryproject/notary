...
func (s *GRPCKeyStore) GenerateKey(keyInfo trustmanager.KeyInfo) (data.PrivateKey, error) {
	...
	if (keyInfo.Role != data.CanonicalRootRole) && (keyInfo.Role != data.CanonicalTargetsRole) {
		logrus.Debugf("grpc key store: generate key supports root and targets role, requested role:%s", keyInfo.Role)
		return nil, fmt.Errorf("grpc key store: supports generating root/target keys, got role %s", keyInfo.Role)
	}
