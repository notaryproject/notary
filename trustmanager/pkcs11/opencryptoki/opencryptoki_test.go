...
// We can add keys enough times to fill up all the slots in the Opencryptoki.
// They are backed up, and we can then list them and get the keys.
func TestOpencryptokiAddKeysAndRetrieve(t *testing.T) {
	...
	// makes sure the keys are actually on the Opencryptoki and not on the original
...
// Test that we can successfully keys enough times to fill up all the slots in the Opencryptoki, even without a backup store
func TestOpencryptokiAddKeysWithoutBackup(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
...
func TestOpencryptokiAddKeyCanAddToMiddleSlot(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
...
// If, when adding a key to the Opencryptoki, we can't back up the key, it should
// be removed from the Opencryptoki too because otherwise there is no way for
// the user to later get a backup of the key.
func TestOpencryptokiAddKeyRollsBackIfCannotBackup(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	// there should be no keys on the Opencryptoki
	require.Len(t, cleanListKeys(t), 0)
}

// If, when adding a key to the Opencryptoki, and it already exists, we succeed
// without adding it to the backup store.
func TestOpencryptokiAddDuplicateKeySucceedsButDoesNotBackup(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	// there should be just 1 key on the Opencryptoki
	require.Len(t, cleanListKeys(t), 1)
	// nothing was added to the backup
	require.Len(t, backup.ListKeys(), 0)
}

// RemoveKey removes a key from the Opencryptoki, but not from the backup store.
func TestOpencryptokiRemoveKey(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
...
// If there are keys in the backup store but no keys in the Opencryptoki,
// listing and getting cannot access the keys in the backup store
func TestOpencryptokiListAndGetKeysIgnoresBackup(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
...
// sign something.
func TestOpencryptokiKeyAndSign(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
...
func testOpencryptokiFunctionCleansUpOnLoginError(t *testing.T, toStub pkcs11Stubbable, functionUnderTest func() error) {
...
func testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t *testing.T,
...
func TestOpencryptokiAddKeyCleansUpOnError(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	testOpencryptokiFunctionCleansUpOnLoginError(t, store, _addkey)
	// all the PKCS11 functions AddKey depends on that aren't the login/logout
	testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t, store, _addkey,
		append(
			setupErrors,
			"FindObjectsInit",
			"FindObjects",
	...
	// the Opencryptoki and no keys in backup
	require.Len(t, backup.ListKeys(), 0)
	require.Len(t, cleanListKeys(t), 0)
...
func TestOpencryptokiGetKeyCleansUpOnError(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t, store, _getkey,
		append(
			setupErrors,
			"FindObjectsInit",
			"FindObjects",
...
func TestOpencryptokiRemoveKeyCleansUpOnError(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	testOpencryptokiFunctionCleansUpOnLoginError(t, store, _removekey)
	// RemoveKey just succeeds if we can't set up the Opencryptoki
	testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t, store, _removekey, setupErrors, false)
	// all the PKCS11 functions RemoveKey depends on that aren't the login/logout
	// or setup/common.Cleanup
	testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t, store, _removekey,
		[]string{
			"FindObjectsInit",
			"FindObjects",
			"FindObjectsFinal",
...
func TestOpencryptokiListKeyCleansUpOnError(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t, store, _listkeys,
		append(
			setupErrors,
			"FindObjectsInit",
			"FindObjects",
...
func TestOpencryptokiSignCleansUpOnError(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
	...
	testOpencryptokiFunctionCleansUpOnLoginError(t, yubiPrivateKey, _sign)
	// all the PKCS11 functions SignKey depends on that is not login/logout
	testOpencryptokiFunctionCleansUpOnSpecifiedErrors(t, yubiPrivateKey, _sign,
		append(
			setupErrors,
			"FindObjectsInit",
			"FindObjects",
...
func TestOpencryptokiRetrySignUntilSuccess(t *testing.T) {
	if !common.IsAccessible() {
		t.Skip("Must have Opencryptoki access.")
	}
	clearAllKeys(t)
...
// of interacting with the Opencryptoki, that everything gets cleaned up sanely.
